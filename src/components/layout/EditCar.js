...
const EditCar = () => {
    ...
    const getCarInfo = () => {
        fetch('http://localhost:9000/get-car', {
            mode: 'cors',
            method: 'POST',
            credentials: 'include',
    ...
    const handleSubmit = () => {
        if(carName !== '' && carPrice !== '' && description !== '' && perCharge !== '' && capacity !== '' && transmission !== ''){
            setLoading(true);
            fetch('http://localhost:9000/update-car', {
                mode: 'cors',
                method: 'POST',
                credentials: 'include',
                headers: {'Content-Type': 'application/json', 'Accept':'application/json', 'Origin':'http://localhost:3000'},
    ...
    const updateImages = () => {
        ...
        if(updatedImages.length == images.length){
            ...
            fetch('http://localhost:9000/update-car-images', {
                mode:'cors',
                method: 'POST',
                credentials: 'include',
                headers: {'Content-Type': 'application/json', 'Accept':'application/json', 'Origin':'http://localhost:3000'},
    ...
    const uploadImages = () => {
        if( newImages !== null && newImages !== [] && newImages.length > 0){
            ...
            fetch('http://localhost:9000/upload-car-images', {
                mode: 'cors',
                method: 'POST',
                credentials: 'include',
