...
const Users = () => {
    ...
    const getUsers = () => {
        fetch('http://localhost:9000/v1/dashboard/users', {credentials: 'include'})
        .then(res => res.json())
        .then(data => {
            if(data.status){
                let u = data.users;
    ...
    const downloadCompletedList = () => {
        fetch('http://localhost:9000/v1/download/completed.csv', {credentials: 'include'})
        .then(res => res.blob())
        .then(blob => {
            setOpenDialog(false);
    ...
    const downloadEmptyCartList = () => {
        fetch('http://localhost:9000/v1/download/empty_cart.csv',{credentials: 'include'})
        .then(res => res.blob())
        .then(blob => {
            setOpenDialog(false);
    ...
    const updateUser = (customerId, status) => {
        fetch('http://localhost:9000/v1/dashboard/update-user', {
            mode: 'cors',
            method: 'POST',
            credentials: 'include',
    ...
    const deleteSelectedUser = () => {
        fetch('http://localhost:9000/v1/dashboard/delete-user',{
            mode: 'cors',
            method: 'POST',
            credentials: 'include',
            headers: {'Content-Type': 'application/json', 'Accept':'application/json', 'Origin': 'http://localhost:3000'},
