...
const Main = () => {
    ...
    const getData = () => {
        fetch('http://localhost:9000/v1/dashboard/main', {credentials: 'include'})
        .then(res => res.json())
        .then(data => {
            if(data.status){
    ...
    const matchingDates = () => {
        ...
        for(let x = 0; days.length > x; x++){
            fetch('http://localhost:9000/v1/dahsboard/days-match', {
                mode: 'cors',
                method: 'POST',
                credentials: 'include',
    ...
    const getPixel = () => {
        fetch('http://localhost:9000/v1/dashboard/pixel', {credentials: 'include'})
        .then(res => res.json())
        .then(data => {
            if(data.status){
                let pieDataArray = [];
    ...
    const getRequests = () => {
        fetch('http://localhost:9000/v1/dashboard/requests', {credentials: 'include'})
        .then(res => res.json())
        .then(data => {
            if(data.status){
   ...
   const checkBookedUsers = () => {
        fetch('http://localhost:9000/check-booked-cars', {
            credentials: 'include'
        }).then(res => res.json())
        .then(data => {
            if(data.status){
